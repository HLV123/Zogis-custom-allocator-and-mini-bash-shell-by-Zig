...
pub fn main() !void {
    ...
    var slab = try SlabAllocator.init(gpa_alloc);
    defer slab.deinit();
    const slab_alloc = slab.allocator();

    var debug_alloc = DebugAllocator.init(slab_alloc);
    ...
    var ctx = executor.ExecContext.init(tracked, &debug_alloc, &slab, &history);
    defer ctx.deinit();
    ...
    while (!ctx.should_exit) {
        ...
        if (ctx.last_exit != 0) {
            try stdout.print("\x1b[31m[{d}]\x1b[0m ", .{ctx.last_exit});
        }
        ...
        var lexer = Lexer.initWithEnv(line, arena_alloc, &ctx.env);
